...
class CLUB(nn.Module):  # CLUB: Mutual Information Contrastive Learning Upper Bound
    ...
    def forward(self, x_samples, y_samples): 
        ...
        prediction_1 = mu
        # print("prediction_1: ", prediction_1.shape)
    ...
    def loglikeli(self, x_samples, y_samples): # unnormalized loglikelihood 
        mu, logvar = self.get_mu_logvar(x_samples.unsqueeze(1))
        # return (-(mu - y_samples.unsqueeze(2))**2 /logvar.exp()-logvar).sum(dim=1).mean(dim=0)
...
if __name__ == "__main__":
    x = torch.randn(16, 189, 512)
    y = torch.randn(16, 187, 512)
    mi_estimator = CLUB(512, 512, 256)
    sampler_loss = mi_estimator(x, y)
    print("sampler_loss: ", sampler_loss.shape)
    ...
    for j in range(5):
        mi_estimator.train()
        mi_loss = mi_estimator.learning_loss(x, y)
        print("mi_loss: ", mi_loss.shape)
