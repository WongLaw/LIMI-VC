...
from models import LIMIVC, get_cosine_schedule_with_warmup

from mi_estimators import CLUB,CLUBSample_reshape
...
def parse_args():
    ...
    parser.add_argument("--milestones", type=int, nargs=2, default=[10000, 15000])
...
def model_fn(batch, model, criterion, self_exclude, ref_included, device, mi_estimator, mi_optimizer):
    ...
    outs, _, tgt, ref1 = model(srcs, refs, src_masks=src_masks, ref_masks=ref_masks)
    # print("tgt, ref1: ", tgt.shape, ref1.shape)
    # ===== may lead to low training speed
    # mi_estimator.eval()
    # mi_loss = mi_estimator(tgt, ref1)
    # mi_loss.backward(retain_graph=True)
    # =====
    for j in range(1):
        mi_estimator.train()
        mi_loss = mi_estimator.learning_loss(tgt, ref1)
...
def valid(dataloader, model, criterion, device, mi_estimator, mi_optimizer):
    ...
    for i, batch in enumerate(dataloader):
        with torch.no_grad():
            loss = model_fn(batch, model, criterion, 1.0, True, device, mi_estimator, mi_optimizer)
...
def main(
    ...
    model = LIMIVC().to(device)
    mi_estimator = CLUB(512,512,512).to(device)
    ...
    mi_optimizer = AdamW(mi_estimator.parameters(), lr=1e-4)
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % save_steps == 0:
            curr_ckpt_name = f"retriever-step{step+1}.pt"
